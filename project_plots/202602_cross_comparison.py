...
renewable_data = pd.DataFrame(
    ...
    columns=pd.MultiIndex.from_product(
        [scenario_name, list_renewables], names=("sce", "tech")
    ),
)
...
for m in listModels:
    for s, variant in scenarios:
        ...
        for var in renewables:
            ...
            for v in var["data"]:
                try:
                    total_v += cross_plots.annualData.loc[
                        (s, variant, m, varName, v, "annual", 2050), "value"
                    ]
                except:
                    total_v = total_v
...
uses_data = pd.DataFrame(
    index=listModels,
    columns=pd.MultiIndex.from_product(
        [scenario_name, list_uses], names=("sce", "use")
    ),
)
